...
CSV를 업로드하면 **웹(Streamlit)** 에서 이상 여부를 테스트할 수 있다. 화면은
**상단 컨트롤 프레임(업로드·EDA/학습 버튼·설정) + EDA 결과 프레임 + 학습 결과 프레임**
3단으로 구성되어 버튼 위치가 고정된다.

- **EDA**: 통계량·분포·상관·클래스 불균형·정상vs이상 비교·오염 경고·**시간 의존성 진단**,
  단계별 **해석 가이드** 포함
- **학습**: 설정 조정(epoch·병목·임계 백분위수·lr)·진행률 표시. **시간 의존성이 높으면
  학습 시 팝업**으로 시계열 윈도우 모델(**LSTM-AE**) vs 행 단위(Dense) 선택
- **학습 후**: 학습 품질(손실곡선·오차분포·**ROC/PR**), 이상 판정(**F1 최대 임계값 추천**),
  개별 샘플 진단, **다중 CSV 배치 판정**, 결과 CSV·**HTML 리포트**·**학습 아티팩트 zip**(사내 이관) 다운로드
