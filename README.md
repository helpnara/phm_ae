...
업로드 후 **EDA**(통계량·분포·상관·클래스 불균형·정상vs이상 비교·오염 경고, 단계별 해석
가이드 포함)와 **학습**(설정 조정·진행률 표시)을 버튼으로 각각 실행한다. 학습 후에는
**학습 품질**(손실곡선·오차분포·ROC/PR), **이상 판정**, **개별 샘플 진단**(피처별 원인),
그리고 **학습 아티팩트 zip 다운로드**(사내 이관용)를 제공한다.
